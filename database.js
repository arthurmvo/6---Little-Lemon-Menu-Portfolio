...
const db = SQLite.openDatabase('little_lemona');
...
export async function filterByQueryAndCategories(query, activeCategories) {
  ...
  const categoriesStr = activeCategories.join("','");
  let sql = '';
  console.log('this is queryStr');
  console.log(queryStr);
  console.log(queryStr.length);
  console.log('this is categoriesStr');
  console.log(categoriesStr);
  console.log(categoriesStr.length);
  switch (true) {
    case queryStr.length > 0 && categoriesStr.length > 0:
      console.log('case 1');
      sql = `SELECT * FROM menuitems WHERE title LIKE '${queryStr}' AND category IN ('${categoriesStr}')`;
      break;
    case queryStr.length > 0 && !categoriesStr.length:
      sql = `SELECT * FROM menuitems WHERE title LIKE '${queryStr}'`;
      break;
    case !queryStr.length && categoriesStr.length > 0:
      sql = `SELECT * FROM menuitems WHERE category IN ('${categoriesStr}')`;
      break;
    default:
